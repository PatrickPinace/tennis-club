...
# ── Generatory meczów — inicjalizacja (RND / SGL / DBE / AMR / MEX) ───────────
# Przeniesione z apps/tournaments/views.py (usuniętego wraz ze starym frontendem
# Django w commicie bea9ce8). Wołane z apps/tournaments/api/views.py przy
# przejściu REG→SCH oraz z endpointu /generate-matches/. Logika bez zmian —
# wyłącznie zmiana lokalizacji + lokalne importy, aby moduł był samowystarczalny.
...
def generate_elimination_matches_initial(tournament, participants_qs, config, bracket_type=None):
    ...
    from apps.tournaments.swiss_logic import _generate_seed_to_slot_map
...
def generate_americano_matches(tournament, participants_qs, config):
    ...
    Uwaga: produkcyjna ścieżka AMR STATIC korzysta z
    generate_americano_matches_static(); ta funkcja obsługuje tryb DYNAMIC
    (Mexicano runda 1) oraz legacy STATIC DBL.
    """
    import random
    from apps.tournaments.models import TournamentsMatch
    ...
    if config.scheduling_type == 'DYNAMIC':
        ...
        # (SNG STATIC używa generate_americano_matches_static przez API)
...
def generate_next_mexicano_round(tournament, config, standings_list):
    ...
    Parowanie w grupie 4 (debel): Team A = (1., 4.), Team B = (2., 3.) — crossover
    poziomów, zgodnie z oficjalnymi zasadami Mexicano.
    """
